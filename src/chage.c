...
static int new_fields (void)
{
	...
	if (a2i(long, &maxdays, buf, NULL, 0, -1,) == -1)
		return 0;
	...
	if (a2i(long, &warndays, buf, NULL, 0, -1,) == -1)
		return 0;

	stprintf_a(buf, "%ld", inactdays);
	change_field(buf, sizeof(buf), _("Password Inactive"));
	if (a2i(long, &inactdays, buf, NULL, 0, -1,) == -1)
		return 0;
...
static void process_flags (int argc, char **argv, struct option_flags *flags)
{
	...
	while ((c = getopt_long (argc, argv, "d:E:hiI:lm:M:R:P:W:",
		...
		case 'I':
			Iflg = true;
			if (a2i(long, &inactdays, optarg, NULL, 0, -1,) == -1) {
		...
		case 'M':
			Mflg = true;
			if (a2i(long, &maxdays, optarg, NULL, 0, -1,) == -1) {
				eprintf(_("%s: invalid numeric argument '%s'\n"),
				         Prog, optarg);
				usage (E_USAGE);
			}
		...
		case 'W':
			Wflg = true;
			if (a2i(long, &warndays, optarg, NULL, 0, -1,) == -1) {
				eprintf(_("%s: invalid numeric argument '%s'\n"),
				         Prog, optarg);
				usage (E_USAGE);
			}
