...
@pytest.mark.topology(KnownTopology.Shadow)
def test_passwd__change_password_as_user_interactive(shadow: Shadow):
    """
    :title: Change user password as user in interactive mode
    :setup:
        1. Create test user
    :steps:
        1. Change user password as root
        2. Change user password as user in interactive mode
        3. Check the user's shadow entry
    :expectedresults:
        1. User's password is changed as root
        2. User's password is changed as user
        3. User's shadow entry is correct
    :customerscenario: False
    """
    shadow.useradd("tuser")

    shadow.passwd("tuser", new_password="Secret123")
    shadow.passwd("", run_as="tuser", old_password="Secret123", new_password="XcqKCymg")
