...
def test_newusers__create_users_from_file(shadow: Shadow):
    ...
    :setup:
        1. Create temporary file with user data in passwd format
        2. Create users using newusers with file input
    :steps:
        1. Check first user passwd entry
        2. Check first user shadow entry
        3. Check first user group entry
        4. Check first user home folder
        5. Check second user passwd entry
        6. Check second user shadow entry
        7. Check second user group entry
        8. Check second user home folder
    :expectedresults:
        1. First user passwd entry exists with correct attributes
        2. First user shadow entry exists with correct attributes
        3. First user group entry exists with correct attributes
        4. First user home folder exists
        5. Second user passwd entry exists with correct attributes
        6. Second user shadow entry exists with correct attributes
        7. Second user group entry exists with correct attributes
        8. Second user home folder exists
    :customerscenario: False
    """
