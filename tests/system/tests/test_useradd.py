...
def test_useradd_default_primary_group_with_supplementary(shadow: Shadow):
    """
    :title: Add a new user with supplementary groups using default primary group
    :steps:
        1. Create required groups
        2. Create user with supplementary group src (default primary group)
        3. Check passwd entry
        4. Check user's own group exists
        5. Verify user is a member of src supplementary group
    :expectedresults:
        1. Groups are created successfully
        2. User is created successfully
        3. passwd entry exists with correct attributes
        4. User's own group (test1) exists with matching GID
        5. User is added to src group as a member
    :customerscenario: False
    """
    shadow.groupadd("src")
    shadow.useradd("test1 -G src")
    username = "test1"
...
def test_useradd_custom_primary_and_supplementary_groups(shadow: Shadow):
    ...
    :steps:
        1. Create required groups
        2. Create user with primary group nogroup and supplementary groups src, daemon, bin
        3. Check passwd entry
        4. Verify primary group is nogroup
        5. Verify user is a member of all supplementary groups
    :expectedresults:
        1. Groups are created successfully
        2. User is created successfully
        3. passwd entry exists with correct attributes
        4. User's primary group is nogroup
        5. User is added to src, daemon, and bin groups as a member
    :customerscenario: False
    """
    groups_to_create = ["nogroup", "src", "daemon", "bin"]
    for group in groups_to_create:
        if shadow.tools.getent.group(group) is None:
            shadow.groupadd(group)

    shadow.useradd("test1 -g nogroup -G src,daemon,bin")
    username = "test1"
    ...
    nogroup_entry = shadow.tools.getent.group("nogroup")
    assert nogroup_entry is not None, "Group nogroup should exist"
    assert (
        passwd_entry.gid == nogroup_entry.gid
    ), f"User's primary GID ({passwd_entry.gid}) should match nogroup GID ({nogroup_entry.gid})"

    supplementary_groups = ["src", "daemon", "bin"]
