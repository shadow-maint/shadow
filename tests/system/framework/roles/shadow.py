...
class Shadow(BaseLinuxRole[ShadowHost]):
    ...
    def newusers(self, *args, users_data: str = None) -> ProcessResult:
        """
    ...
    def groupmems(self, *args, user: str = "root", password: str = None) -> ProcessResult:
        """
        Administer members of a user's primary group.

        The groupmems command allows management of group membership lists.
        If `user` is provided, then the `password` must be provided and the command groupmems run under this user.
        If `user` isn't provided, then the command is run under `root`.
        """
        args_dict = self._parse_args(args)
        ...
        if user == "root":
            ...
            cmd = self.host.conn.run(f"echo '{password}' | su - {user} -c 'groupmems {args[0]}'", log_level=ProcessLogLevel.Error)
    ...
    def newgrp(self, *args, user: str = "root") -> Tuple[ProcessResult, int]:
        """
        ...
        self.logger.info(f'Changing {user} to group "{args_dict["name"]}" on {self.host.hostname}')
