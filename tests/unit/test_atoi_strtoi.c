...
test_strtoi(void **state)
{
	...
	errno = 0;
	assert_true(STRTON(intmax_t, "42", NULL, -1, 1, 2, &status) == 1);
	assert_true(status == EINVAL);

	assert_true(STRTON(intmax_t, "42", NULL, 1, 1, 2, &status) == 1);
	assert_true(status == EINVAL);

	assert_true(STRTON(intmax_t, "40", &end, 5, INTMAX_MIN, INTMAX_MAX, &status) == 20);
	assert_true(status == 0);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(intmax_t, "-40", &end, 0, INTMAX_MIN, INTMAX_MAX, &status) == -40);
	assert_true(status == 0);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(intmax_t, "z", &end, 0, INTMAX_MIN, INTMAX_MAX, &status) == 0);
	assert_true(status == ECANCELED);
	assert_true(strcmp(end, "z") == 0);

	assert_true(STRTON(intmax_t, " 5", &end, 0, 3, 4, &status) == 4);
	assert_true(status == ERANGE);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(intmax_t, "5z", &end, 0, INTMAX_MIN, INTMAX_MAX, &status) == 5);
	assert_true(status == ENOTSUP);
	assert_true(strcmp(end, "z") == 0);

	assert_true(STRTON(intmax_t, "5z", &end, 0, INTMAX_MIN, 4, &status) == 4);
	assert_true(status == ERANGE);
	assert_true(strcmp(end, "z") == 0);
...
test_strtou(void **state)
{
	...
	errno = 0;
	assert_true(STRTON(uintmax_t, "42", NULL, -1, 1, 2, &status) == 1);
	assert_true(status == EINVAL);

	assert_true(STRTON(uintmax_t, "42", NULL, 1, 1, 2, &status) == 1);
	assert_true(status == EINVAL);

	assert_true(STRTON(uintmax_t, "40", &end, 5, 0, UINTMAX_MAX, &status) == 20);
	assert_true(status == 0);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(uintmax_t, "-40", &end, 0, 0, UINTMAX_MAX, &status) == -40ull);
	assert_true(status == 0);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(uintmax_t, "z", &end, 0, 0, UINTMAX_MAX, &status) == 0);
	assert_true(status == ECANCELED);
	assert_true(strcmp(end, "z") == 0);

	assert_true(STRTON(uintmax_t, " 5", &end, 0, 3, 4, &status) == 4);
	assert_true(status == ERANGE);
	assert_true(strcmp(end, "") == 0);

	assert_true(STRTON(uintmax_t, "5z", &end, 0, 0, UINTMAX_MAX, &status) == 5);
	assert_true(status == ENOTSUP);
	assert_true(strcmp(end, "z") == 0);

	assert_true(STRTON(uintmax_t, "5z", &end, 0, 0, 4, &status) == 4);
	assert_true(status == ERANGE);
	assert_true(strcmp(end, "z") == 0);
